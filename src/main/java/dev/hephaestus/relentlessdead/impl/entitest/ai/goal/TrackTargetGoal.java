...
import dev.hephaestus.relentlessdead.impl.entitest.ai.pathing.PathNode;
...
public abstract class TrackTargetGoal extends Goal {
	...
	public TrackTargetGoal(PathfindingEntity mob, boolean checkVisibility) {
		this(mob, checkVisibility, false);
	}

	public TrackTargetGoal(PathfindingEntity mob, boolean checkVisibility, boolean checkNavigable) {
	...
	protected boolean canTrack(@Nullable LivingEntity target, TargetPredicate targetPredicate) {
		if (target == null) {
			return false;
		} else if (!targetPredicate.test((LivingEntity) this.mob, target)) {
			return false;
	...
	private boolean canNavigateToEntity(LivingEntity entity) {
		...
		Path path = this.mob.getZombieNavigation().findPathTo(entity, 0);
		if (path == null) {
			return false;
		} else {
