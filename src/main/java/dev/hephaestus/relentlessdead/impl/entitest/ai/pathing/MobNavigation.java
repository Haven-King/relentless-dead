...
public class MobNavigation extends EntityNavigation {
   ...
   public MobNavigation(PathfindingEntity mobEntity, World world) {
         ...
         for(int k = i; k < i + xSize; ++k) {
            for(int l = j; l < j + zSize; ++l) {
               ...
               if (d * lookVecX + e * lookVecZ >= 0.0D) {
                  ...
                  if (pathNodeType == PathNodeTypes.DAMAGE_FIRE || pathNodeType == PathNodeTypes.DANGER_FIRE || pathNodeType == PathNodeTypes.DAMAGE_OTHER) {
                     return false;
                  }
               }
            }
   ...
   protected boolean canWalkOnPath(PathNodeType pathType) {
      if (pathType == PathNodeTypes.WATER) {
         return false;
      } else if (pathType == PathNodeTypes.LAVA) {
         return false;
      } else {
         return pathType != PathNodeTypes.OPEN;
      }
   }
