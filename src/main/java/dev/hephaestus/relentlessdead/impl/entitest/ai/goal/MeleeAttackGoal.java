...
import dev.hephaestus.relentlessdead.impl.entitest.ai.pathing.Path;
...
public class MeleeAttackGoal extends Goal {
	protected final PathfindingEntity mob;
	...
	public MeleeAttackGoal(PathfindingEntity mob, double speed, boolean pauseWhenMobIdle) {
		this.mob = mob;
		this.speed = speed;
	...
	public boolean canStart() {
		...
		if (l - this.lastUpdateTime < 20L) {
			...
			if (livingEntity == null) {
				...
				this.path = this.mob.getZombieNavigation().findPathTo((Entity)livingEntity, 0);
				if (this.path != null) {
					return true;
				} else {
					return this.getSquaredMaxAttackDistance(livingEntity) >= this.mob.squaredDistanceTo(livingEntity);
				}
			}
		}
	}
	...
	public boolean shouldContinue() {
		...
		if (livingEntity == null) {
			...
			return !this.mob.getZombieNavigation().isIdle();
	...
	public void start() {
		this.mob.getZombieNavigation().startMovingAlong(this.path, this.speed);
	...
	public void stop() {
		...
		this.mob.getZombieNavigation().stop();
	}
	...
	public void tick() {
		...
		if ((this.pauseWhenMobIdle || this.mob.getVisibilityCache().canSee(livingEntity)) && this.updateCountdownTicks <= 0 && (this.targetX == 0.0D && this.targetY == 0.0D && this.targetZ == 0.0D || livingEntity.squaredDistanceTo(this.targetX, this.targetY, this.targetZ) >= 1.0D || this.mob.getRandom().nextFloat() < 0.05F)) {
			...
			if (!this.mob.getZombieNavigation().startMovingTo(livingEntity, this.speed)) {
